...
class CallSession extends Emitter {
  constructor(logger, mrf, req, res) {
    ...
    if (process.env.TTS_VENDOR && process.env.TTS_LANGUAGE && process.env.TTS_VOICE) {
      ...
      this.voice = process.env.TTS_VOICE;
    }
    else {
      this.useTts = false;
    }

    if (process.env.LEX_WELCOME_MESSAGE) {
      this.welcomeMessage = process.env.LEX_WELCOME_MESSAGE;
    }
  ...
  hangup() {
    ...
    if (this.dlg) this.dlg.destroy().catch((err) => {});
    if (this.ep) this.ep.destroy().catch((err) => {});
    this._clearTmpFiles();
  }
